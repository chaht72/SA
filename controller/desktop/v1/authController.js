...
const {
  SaqibEstate,role,userRole,userAuthSettings,userTokens,pushNotification
} = require('../../../model/index');
...
const register = async (req, res) => {
  try {
    ...
    let checkUniqueFields = await checkUniqueFieldsInDatabase(SaqibEstate,[ 'username' ],dataToRegister,'INSERT');
    ...
    const result = await dbService.createOne(SaqibEstate,{
      ...dataToRegister,
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    where.isActive = true;where.isDeleted = false;        let found = await dbService.findOne(SaqibEstate,where);
    if (!found) {
...
const loginWithOTP = async (req, res) => {
  ...
  try {
    ...
    let where = { 'username':params.username };
    where.isActive = true;where.isDeleted = false;        let User = await dbService.findOne(SaqibEstate,where);
    if (!User) {
      // invalid code
    ...
    let result = await authService.loginWithOTP(params.username, null, authConstant.PLATFORM.DESKTOP,roleAccess);
    if (result.flag){
