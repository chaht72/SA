...
const softDeleteUser = async (req, res) => {
  try {
    ...
    let result = await dbService.update(User, query,updateBody);
    if (!result){
      return res.recordNotFound();
    }
...
const deleteUser = async (req, res) => {
  const result = await dbService.deleteByPk(User, req.params.id);
  return  res.success({ data :result });
};
...
const deleteManyUser = async (req, res) => {
  try {
    ...
    let deletedUser = await dbService.destroy(User,query);
    return res.success({ data :{ count :deletedUser.length } });
  }
  catch (error){
    return res.internalServerError({ message:error.message });  
  }
...
const softDeleteManyUser = async (req, res) => {
  try {
    ...
    const options = {};
    let updatedUser = await dbService.update(User,query,updateBody, options);
    if (!updatedUser) {
      return res.recordNotFound();
    }
    return  res.success({ data :{ count: updatedUser.length } });
