...
const sendEmailForLoginOtp = async (user) => {
  try {
    ...
    let updatedUser = await dbService.findOne(model.SaqibEstate,where);
...
const loginUser = async (username,password,platform,roleAccess) => {
  try {
    ...
    where.isActive = true;where.isDeleted = false;            const user = await dbService.findOne(model.SaqibEstate,where);
    if (!user) {
      return {
        flag:true,
...
const changePassword = async (params)=>{
  try {
    ...
    where.isActive = true;where.isDeleted = false;        let user = await dbService.findOne(model.SaqibEstate,where);
    if (!user || !user.id) {
      return {
        flag:true,
        data:'User not found'
    ...
    let updatedUser = dbService.update(model.SaqibEstate,where,{ password:password });
    if (!updatedUser) {
      return {
        flag:true,
...
const resetPassword = async (userId, newPassword) => {
  try {
    let where = { id: userId };
    where.isActive = true;where.isDeleted = false;        const dbUser = await dbService.findOne(model.SaqibEstate,where);
    if (!dbUser) {
      return {
        flag: true,
        data: 'User not found',
    ...
    let updatedUser = await dbService.update(model.SaqibEstate, where, { password: newPassword });
    if (!updatedUser) {
      return {
        flag: true,
...
const socialLogin = async (email,platform) => {
  try {
    const user = await dbService.findOne(model.SaqibEstate,{ email });
    if (user && user.email) {
      const { ...userData } = user.toJSON();
      if (!user.userType) {
        return {
